        ...
        <nav className={`bg-black flex items-center p-5 justify-between overflow-hidden ${isClick ? 'fixed  inset-0 z-50 flex-col justify-start' : ''}`}>
            ...
            <div className='lg:hidden items-center'>
                <button
                    ...
                    hover:text-white focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white transition-transform'
            ...
            {isClick && (
                <div className='absolute inset-x-0 top-20 flex items-center'>
                    <div className='p-16 sm:px-3 rounded-lg text-xl font-mono w-full '>
                        <Link href={`/`} legacyBehavior><a className="hover:text-yellow-200 hover:scale-110 transition duration-300 block p-3 rounded-2xl">INICIO</a></Link>

                        <Link href="#info" legacyBehavior><a className="hover:text-yellow-200 hover:scale-110 transition duration-300 block mt-8  p-3 rounded-2xl">INFO</a></Link>

                        <Link href="#news" legacyBehavior><a className="hover:text-yellow-200 hover:scale-110 transition duration-300 block mt-8  p-3 rounded-2xl">NOTICIAS</a></Link>

                        <Link href="#proyecto" legacyBehavior><a className="hover:text-yellow-200 hover:scale-110 transition duration-300 block mt-8 p-3 rounded-2xl">PROYECTOS</a></Link>

                        {session?.user ? (
                            <Link href={`/users`} legacyBehavior><a className="hover:text-yellow-200 hover:scale-110 transition duration-300 block mt-8  p-3 rounded-2xl">MIEMBROS</a></Link>
                        ) : (
                            <div></div>
                        )}
                    </div>
